...
if [ ! -e "$DataName.$CutoffCode.Filter03.recode.fixed.vcf" ]; then
	...
	vcftools --vcf $DataName.$CutoffCode.Filter02.recode.vcf --minQ 400 --recode --recode-INFO-all --out $DataName.$CutoffCode.Filter03
...
echo
echo `date` "---------------------------FILTER06: Remove sites without Allele Balance near 0, 0.5 or 1 -----------------------------"

if [ ! -e "$DataName.$CutoffCode.Filter06.vcf" ]; then
	# Remove sites without balance near 0, 0.5, or 1 (i.e. Heterozygote or Homozygote)  
	vcffilter -s -f "AB > 0.2 & AB < 0.8 | AB < 0.05 | AB > 0.95" $DataName.$CutoffCode.Filter05.recode.vcf | vcffixup - > $DataName.$CutoffCode.Filter06.vcf
...
if [ ! -e "$DataName.$CutoffCode.Filter07.vcf" ]; then
	...
	vcffilter -f "AC > 1" $DataName.$CutoffCode.Filter06.vcf > $DataName.$CutoffCode.Filter07.vcf
...
if [ ! -e "$DataName.$CutoffCode.Filter09.vcf" ]; then
	...
	vcffilter -f "MQM / MQMR > 0.9 & MQM / MQMR < 1.05" $DataName.$CutoffCode.Filter08.vcf > $DataName.$CutoffCode.Filter09.vcf
...
if [ ! -e "$DataName.$CutoffCode.Filter14.recode.vcf" ]; then
	...
	vcftools --vcf $DataName.$CutoffCode.Filter13.recode.vcf --minDP 3 --recode --recode-INFO-all --out $DataName.$CutoffCode.Filter14
else
	echo "Using previously created file"
