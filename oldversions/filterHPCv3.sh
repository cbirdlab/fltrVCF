...
#SBATCH --job-name=frOp25.10
#SBATCH --output=output_25.10.K.out
#SBATCH -p cbirdq
...
vcffilter -f "AB > 0.25 & AB < 0.75" $MAPdir/TotalRawSNPs.$CutoffCode.vcf | vcfallelicprimitives --keep-info --keep-geno | vcffixup - | vcffilter -f "AC > 2" > $DataName.$CutoffCode.Filter01.vcf
...
echo
echo `date` "---------------------------FILTER0003: Remove Sites With Rare Variants-----------------------------"
...
vcftools --vcf $DataName.$CutoffCode.Filter03.recode.vcf --max-missing 0.5 --minQ 30 --recode --recode-INFO-all --out $DataName.$CutoffCode.Filter04 
...
vcftools --vcf $DataName.$CutoffCode.Filter06.recode.vcf --max-missing 0.75 --maf 0.002 --max-maf 0.998 --recode --recode-INFO-all --out $DataName.$CutoffCode.Filter07 --min-meanDP 20
...
echo
echo `date` "---------------------------FILTER009-----------------------------"
#CEB Need to fix for MNPs.  This will erroneously remove good snps from MNPs because 
...
vcftools --vcf  $DataName.$CutoffCode.Filter13.vcf --recode-INFO-all --out $DataName.$CutoffCode.Filter14 --max-meanDP 300 --exclude-positions $DataName.$CutoffCode.Filter13.lowQDloci --recode    
...
#perl rad_haplotyper115HPC.pl -v $DataName.$CutoffCode.Filter15.HWE.recode.vcf -x $NumProc -e -mp 5 -u 20 -ml 40 -h 300 -d 50 -z 4 -m .5 -n -r $MAPdir/reference.$CutoffCode.fasta -bp $MAPdir -co $CutoffCode -dn $DataName  -p $MAPdir/popmap.$CutoffCode$Pops -o $DataName.$CutoffCode.Filter16.Haplotyped.vcf -g $DataName.$CutoffCode$Pops.haps.genepop -a $DataName.$CutoffCode$Pops.haps.ima  
