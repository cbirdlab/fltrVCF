...
df$totalGenotypes <- rowSums(df[13:19])
...
df$PropGenoGreater999X <- df$NumGenoGreater999X  / df$totalGenotypes

#stack data
df_contigs <- data.frame(df[20], stack(df[5:9]))
colnames(df_contigs) <- c("FilterID_Order", "NumContigs", "ContigType")

df_variants <- data.frame(df[20], stack(df[10:12]))
colnames(df_variants) <- c("FilterID_Order", "NumVariantPos", "VariantType")

df_cvgNum <- data.frame(df[20], stack(df[13:19]))
colnames(df_cvgNum) <- c("FilterID_Order", "NumGenotypes", "Depth")

df_cvgProp <- data.frame(df[20], stack(df[22:28]))
...
plotNumContigs <- ggplot(data=df_contigs, aes(x=FilterID_Order, y=NumContigs, color=ContigType, group=ContigType)) +
  geom_line() +
  geom_point() +
  theme_classic() +
  theme(axis.text.x = element_text(angle=45, hjust=1))

plotNumVariantPos <- ggplot(data=df_variants, aes(x=FilterID_Order, y=NumVariantPos, color=VariantType, group=VariantType)) +
  geom_line() +
  geom_point() +
  theme_classic() +
  theme(axis.text.x = element_text(angle=45, hjust=1))
