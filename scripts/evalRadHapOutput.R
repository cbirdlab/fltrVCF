...
#df <- read.table("OpihiSK2017_EastMaui.D4.5.5.Fltr19.ind_stats.out", header=TRUE)
df <- read.table("OpihiSK2017_EastMaui.C.Plate2.5.5.Fltr19.ind_stats.out", header=TRUE)
df <- read.table("OpihiSK2017_EastMaui.B.Plate3.5.5.Fltr19.ind_stats.out", header=TRUE)
...
ggplot(data=df5, aes(x=Low_Coverage.Errors, y=Poss_Paralogs, color=NGS_Library)) +
  geom_point() +
  geom_smooth(se=FALSE) +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
